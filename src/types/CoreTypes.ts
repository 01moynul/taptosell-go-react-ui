...
/**
 * Interface for a User (Dropshipper, Supplier, Manager, Admin).
 * Corresponds to Go model models.User.
 */
export interface User {
    id: number;
    email: string;
    fullName: string;
    role: 'dropshipper' | 'supplier' | 'manager' | 'administrator';
    status: 'unverified' | 'pending' | 'active' | 'suspended' | 'rejected';
    phoneNumber?: string;
    companyName?: string;
    /**
     * The number of penalty strikes accumulated by the user (mostly for Dropshippers).
     * Used for the Penalty Strike System (Phase 8.6).
     */
    penaltyStrikes: number; 
}

/**
 * Interface for the response when fetching a list of users (Manager view).
 * API: GET /v1/manager/users (To be implemented)
 */
...
/**
 * Payload for manually updating a user's penalty strikes.
 * API: PATCH /v1/manager/users/:id/penalty
 */
...
/**
 * Interface for a single message in the AI Chat UI.
 * Used for frontend state management to display the conversation history.
 */
export interface AIChatMessage {
    id: string; // Unique ID for React keys (e.g., generic UUID or timestamp)
    role: 'user' | 'ai'; 
    text: string;
    timestamp: Date;
    isError?: boolean; // Optional flag to indicate if the message failed to send
}

/**
 * Payload for sending a message to the Hybrid AI Assistant.
 * API: POST /v1/ai/chat
 */
 ...
 * API: POST /v1/ai/chat
 * Includes the AI's text response and the usage-based billing details.
 */
export interface AIChatResponse {
    response: string; // The Markdown-formatted text response from the AI
    creditsUsed: number; // The cost deducted for this specific interaction
    creditsRemaining: number; // The user's new updated balance
}
