...
// import { type ProductPayload } from '../../services/supplierProductService'; // Removed as it was unused
...
const VariationManager: React.FC<VariationManagerProps> = ({ onVariationsChange, initialVariations, isEditing }) => {
    ...
    const calculateVariants = useCallback((currentGroups: VariationGroup[], existingVariants: Variant[]): Variant[] => { // <-- ADD existingVariants parameter
        ...
        const newVariants: Variant[] = combinations.map(combo => {
            ...
            const existingVariant = existingVariants.find(v => JSON.stringify(v.combination) === comboKey); // <-- USE existingVariants
        ...
        return newVariants;
    }, []); // <-- REMOVE [variants] dependency. The function is now static.
    
    useEffect(() => {
        // Now pass the current variants state as the second argument
        const newVariants = calculateVariants(groups, variants); 
        
        // CRITICAL CHECK: Only update state if the new variants array is actually different
        // This prevents the re-render when a group is added but no options exist yet, etc.
        if (JSON.stringify(newVariants) !== JSON.stringify(variants)) {
            setVariants(newVariants);
            onVariationsChange(newVariants);
        }
        
    // REMOVED [variants] from the dependency list to stop the infinite loop.
    // The effect now only runs when the input data (`groups`) or handlers change.
    }, [groups, calculateVariants, onVariationsChange]); // Dependencies look correct to ensure reactiveness
