...
const VariationManager: React.FC<VariationManagerProps> = ({ onVariationsChange, initialVariations, isEditing }) => {
    ...
    // Effect to recalculate variants whenever groups change and push data to parent (FIXED: Breaks infinite loop)
    useEffect(() => {
        // Pass the current variants state explicitly
        const newVariants = calculateVariants(groups, variants);
        
        // CRITICAL FIX: Use a deep comparison (stringification) before setting state.
        // This prevents re-render if the calculated list is logically the same, breaking the infinite loop.
        if (JSON.stringify(newVariants) !== JSON.stringify(variants)) {
            setVariants(newVariants);
            onVariationsChange(newVariants);
        }
        
    }, [groups, calculateVariants, onVariationsChange]); // FIXED: Removed 'variants' dependency
