...
const VariationManager: React.FC<VariationManagerProps> = ({ onVariationsChange, initialVariations, isEditing }) => {
    ...
    useEffect(() => {
        ...
        if (isEditing && initialVariations && Array.isArray(initialVariations)) {
            // NOTE: A robust implementation would parse the 'initialVariations' 
            // array (which contains Variant objects) and reverse-engineer the unique 
            // VariationGroups (e.g., extracting "Size" and all unique "S", "M" values).
            
            // For now, we rely on the user adding groups in edit mode if needed, 
            // as the complex reverse-engineering logic is outside the scope of this fix.
            // We will proceed with an empty state for simplicity during this phase.
        }
    }, [isEditing, initialVariations]);

    // --- Core Logic: Cartesian Product Calculation (FIXED: Removed 'variants' dependency) ---

    // Function to calculate all possible combinations and generate the variants table
    // It takes the existing variants list as an argument, making the function static.
    const calculateVariants = useCallback((currentGroups: VariationGroup[], existingVariants: Variant[]): Variant[] => {
        ...
        return newVariants;
    }, []); // FIXED: Dependency array is empty now.
    
    // The key is that this effect now ONLY depends on the inputs: 'groups' and the handler functions.
    useEffect(() => {
        
        // 1. Calculate the new variants list based on the current groups and the current variants state.
        const newVariants = calculateVariants(groups, variants);

        // 2. Functional Update: Use the setter form (setVariants(prevVariants => ...))
        // This lets us compare the newly calculated list against the official previous state (prevVariants)
        // inside the safe scope of the functional update.
        setVariants(prevVariants => {
            // Compare the newly calculated list (newVariants) against the previous state (prevVariants)
            if (JSON.stringify(newVariants) !== JSON.stringify(prevVariants)) {
                
                // If they are different, we set the new state and notify the parent.
                onVariationsChange(newVariants);
                return newVariants; // Return the new state
            }
            
            // If they are the same, return the old state (do nothing).
            return prevVariants; // Return the previous state
        });
        
    // The dependency list is clean and only includes the components that trigger the calculation.
