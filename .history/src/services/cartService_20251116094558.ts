...
/**
 * @description Calls the PUT /v1/dropshipper/cart/items/:product_id endpoint to update item quantity.
 * @param {number} productId - The ID of the product in the cart.
 * @param {number} quantity - The new quantity.
 * @returns {Promise<void>} - Resolves on success.
 */
export const updateCartItem = async (productId: number, quantity: number): Promise<void> => {
  // PUT /v1/dropshipper/cart/items/:product_id
  await apiClient.put(`/dropshipper/cart/items/${productId}`, { quantity });
};


/**
 * @description Calls the DELETE /v1/dropshipper/cart/items/:product_id endpoint to remove an item.
